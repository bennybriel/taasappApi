from .. serializers import LevelUploadSerializer
...
class LevelFileUploaderView(APIView):
    serializer_class = LevelUploadSerializer
    @swagger_auto_schema(request_body=LevelUploadSerializer)
    def post(self, request, *args, **kwargs):    
        serializer =LevelUploadSerializer(data=request.data)
        ...
        try:
            ...
            for _, row in reader.iterrows():
                ...
                ck = Level.objects.filter(name = row['name']).filter(userID = userID)
                if not ck and session_check : new_file.save()
                
            recordCounter = Level.objects.filter(userID = request.data['userID']).count()            
